...
# Add scripts/ to path so we can import the main dataset utilities
_SCRIPTS_DIR = pathlib.Path(__file__).parent.parent / "scripts"
sys.path.insert(0, str(_SCRIPTS_DIR))
import generate_addition_dataset as gen  # noqa: E402

# ─────────────────────────────────────────────────────────────────────────────
# Alphabet filler: cycles a-z A-Z, 52 unique letters.
# Space-separated → 53 unique token types in Qwen's tokenizer.
# ─────────────────────────────────────────────────────────────────────────────
...
def _build_alphabet_filler(filler_len: int) -> str:
    """Cycle through a-z A-Z for filler_len steps."""
    return " ".join(_ALPHABET[i % len(_ALPHABET)] for i in range(filler_len))


# Monkey-patch gen._build_filler to add "alphabet" support.
# Module-level functions look up names in their module's __dict__ at call time,
# so patching gen._build_filler affects build_filler_response, build_filler_prefill,
# and DatasetBuilder — all of which call gen._build_filler internally.
_orig_build_filler = gen._build_filler
