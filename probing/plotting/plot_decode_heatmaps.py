...
for metric, cmap, label, vmin, vmax, fname in [
    ...
    fig, axes = plt.subplots(1, n_conds, figsize=(max(5, n_conds * 1.2 + 2), 10),
                              sharey=True)
    if n_conds == 1:
        axes = [axes]
    ...
    for ax, cond in zip(axes, cond_order):
        ...
        ax.set_xticks([0])
        ax.set_xticklabels(["avg_filler"], fontsize=9)
    ...
    fig.suptitle(f"Unsupervised decode (avg filler): {label}", fontsize=13)
...
# --- Per-position × layer heatmaps (3-panel) ---
...
for actual_metric, cmap, label, vmin, vmax, fname in [
    ...
    conds_to_plot = [c for c in pp_order if c in pp_data]
    if not conds_to_plot:
        continue

    fig, axes = plt.subplots(1, len(conds_to_plot),
                              figsize=(5 * len(conds_to_plot) + 2, 10))
    ...
    fig.suptitle(f"Unsupervised decode per position: {label}", fontsize=14)
