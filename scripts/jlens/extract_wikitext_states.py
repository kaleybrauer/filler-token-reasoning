...
def main():
    ...
    print(f"corpus {corpus['source']}  prompts {lo}..{hi-1} (held out)", flush=True)
    ...
    print(f"  will store {len(prompts)} x {T} positions x {len(layers)} layers "
    ...
    print(f"\nwrote {args.out} ({args.out.stat().st_size/2**30:.1f} GiB) "
