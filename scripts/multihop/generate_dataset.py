...
Copied from Ryan Greenblatt's multi_hop repository:
https://github.com/rgreenblatt/multi_hop
Minor modification: generate_dataset_constants.py uses __file__-relative
paths for data files so this package works from any working directory.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Suffix to append to output filenames before .jsonl (e.g. '_900' → problems_2hop_900.jsonl)",
    )
    ...
    save_dataset(all_2hop, f"{prefix}problems_2hop{sfx}.jsonl")
    save_dataset(all_3hop, f"{prefix}problems_3hop{sfx}.jsonl")
    save_dataset(all_4hop, f"{prefix}problems_4hop{sfx}.jsonl")

    # Save combined dataset
    all_problems = all_2hop + all_3hop + all_4hop
    save_dataset(all_problems, f"{prefix}problems_all{sfx}.jsonl")
