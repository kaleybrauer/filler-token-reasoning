"""
pool_decode_global_dedup.py

Unsupervised dual-variable recovery via global agreement ranking + greedy dedup.

Pipeline:
1. For each correct example, decode at every (position, layer) in filler region via
   logit lens (RMSNorm → lm_head → argmax over number tokens).
2. Filter candidates: pos ≤ filler_end, layer ≥ --min-layer, diversity ≥ --min-diversity.
3. Compute pairwise agreement (fraction of examples with identical predictions) between
   all candidates.
4. Rank by mean global agreement (average pairwise agreement with all others).
5. Greedy dedup: walk through ranked list, keep setting if its max agreement with any
   already-kept setting is < --dedup-threshold. Stop at --n-kept.
6. Pool softmax probabilities across kept settings.
7. Report top-K recovery: fraction of examples where A₁, A₂, and both appear in the
   pooled top-K predictions.

Usage:
    python scripts/pool_decode_global_dedup.py --condition dots_10
    python scripts/pool_decode_global_dedup.py --condition counting_10 --n-kept 15
"""
...
def main():
    ...
    # Candidate (position, layer) pairs
    all_positions = sorted([p for p in d0["states"] if p.startswith("pos_")],
                            key=lambda p: int(p.split("_")[1]))

    # Step 1+2: decode everywhere, filter
    candidates = []  # (pos, layer, predictions)
    ...
    # Step 7: top-K recovery
    sort_idx = np.argsort(pooled, axis=1)[:, ::-1]
    top_preds = num_vals[sort_idx]

    print(f"\nPooled top-K recovery ({len(kept)} settings):")
    print(f"  {'K':>4}  {'A1':>5}  {'A2':>5}  {'Both':>5}")
    results = {}
    for K in args.top_k:
        a1 = float(np.mean([A1[i] in top_preds[i, :K] for i in range(n)]))
        a2 = float(np.mean([A2[i] in top_preds[i, :K] for i in range(n)]))
        both = float(np.mean([(A1[i] in top_preds[i, :K]) and (A2[i] in top_preds[i, :K])
                              for i in range(n)]))
        results[f"top_{K}"] = {"a1": a1, "a2": a2, "both": both}
        print(f"  {K:>4}  {a1:>4.0%}  {a2:>4.0%}  {both:>4.0%}")

    # Save
    out = args.output or Path(f"results/pool_decode_global_{args.condition}.json")
